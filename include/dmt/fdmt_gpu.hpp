...
struct FDMTPlanD {
    ...
    // in vector "len_inner_vects_coordinates_cumsum" we store cummulative sums
    // of elements  inner vectors of "coordinates" So,
    // len_inner_vects_coordinates_cumsum[0] = 0
    // len_inner_vects_coordinates_cumsum[1] =
    // len_inner_vects_coordinates_cumsum[0] + coordinates[0].size() *2
    // ...
    // len_inner_vects_coordinates_cumsum[n] =
    // len_inner_vects_coordinates_cumsum[n-1] + coordinates[n-1].size() *2
    // ...
    // Remember that always:  len_inner_vects_coordinates_cumsum.size() =
    // NUmIter +1
    ...
    // in vector "len_mappings_cumsum" we store cummulative sums of elements
    // inner vectors of "mappings" So, len_mappings_cumsum[0] = 0
    // len_mappings_cumsum[1] = len_mappings_cumsum[0] + mappings[0].size() *5
    // ...
    // len_mappings_cumsum[n] = len_mappings_cumsum[n-1] + mappings[n-1].size()
    // *5
    // ...
...
__global__ void kernel_init_fdmt(const float* __restrict__ waterfall,
                                 float* __restrict__ state,
                                 const int* __restrict__ p_state_sub_idx,
                                 const int* __restrict__ p_dt_grid,
                                 const int* __restrict__ p_pos_gridInnerVects,
                                 int num_sub,
                                 int nsamps);
