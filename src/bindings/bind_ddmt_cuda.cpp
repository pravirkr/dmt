...
void bind_ddmt_cuda(py::module_& mod) {
    py::class_<DDMTCUDA>(mod, "DDMTCUDA",
        ...
        .def(py::init([](float f_min, float f_max, SizeType nchans, float tsamp,
                 ...
                 return DDMTCUDA(f_min, f_max, nchans, tsamp, dm_max, dm_step,
                                 dm_min, device_id, nbits, km_vec, nbeams);
             }),
             "f_min"_a, "f_max"_a, "nchans"_a, "tsamp"_a, "dm_max"_a,
             "dm_step"_a, "dm_min"_a = 0.0F, "device_id"_a = 0, "nbits"_a = 32,
             "kill_mask"_a = py::none(), "nbeams"_a = 1)
        .def(py::init([](float f_min, float f_max, SizeType nchans, float tsamp,
                         const py::array_t<float>& dm_arr, int device_id,
                         SizeType nbits,
                         std::optional<py::array_t<uint8_t>> kill_mask,
                         SizeType nbeams) {
                 std::vector<uint8_t> km_vec;
        ...
        .def(
            py::init([](float f_min, float f_max, SizeType nchans, float tsamp,
                        const LevinConfig& levin, int device_id, SizeType nbits,
                        std::optional<py::array_t<uint8_t>> kill_mask,
                        SizeType nbeams) {
                std::vector<uint8_t> km_vec;
                if (kill_mask.has_value()) {
                ...
                return DDMTCUDA(f_min, f_max, nchans, tsamp, levin, device_id,
                                nbits, km_vec, nbeams);
            }),
        ...
        .def(
            "load_history",
            [](DDMTCUDA& ddmt, const py::array& hist_obj) {
                const auto& plan = ddmt.get_plan();
