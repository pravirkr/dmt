...
TEST_CASE("cff", "[fdmt_utils]") {
    REQUIRE(dm_utils::cff(1000.0F, 1500.0F, 1000.0F, 1500.0F) == 1.0F);
    REQUIRE(dm_utils::cff(1500.0F, 1000.0F, 1500.0F, 1000.0F) == 1.0F);
    REQUIRE(dm_utils::cff(1000.0F, 1000.0F, 1000.0F, 1500.0F) == 0.0F);
}

TEST_CASE("calculate_dt_sub", "[fdmt_utils]") {
    REQUIRE(dm_utils::calculate_dt_sub(1000.0F, 1500.0F, 1000.0F, 1500.0F,
                                       100) == 100);
    REQUIRE(dm_utils::calculate_dt_sub(1000.0F, 1500.0F, 1000.0F, 1500.0F, 0) ==
            0);
}
...
TEST_CASE("add_offset_kernel", "[fdmt_utils]") {
    SECTION("Test case 1: Valid input and output vectors") {
        ...
        size_t offset = 2;
        REQUIRE_NOTHROW(dm_utils::add_offset_kernel<true>(
    ...
    SECTION("Test case 2: Output size less than input size") {
        ...
        size_t offset = 2;
        REQUIRE_THROWS_AS(dm_utils::add_offset_kernel<true>(
                              arr1.data(), arr1.size(), arr2.data(),
                              arr2.size(), arr_out.data(), arr_out.size(),
                              offset),
                          std::runtime_error);
    }
...
TEST_CASE("find_closest_index", "[fdmt_utils]") {
    SECTION("Test case 1: Empty array") {
        ...
        REQUIRE_THROWS_AS(dm_utils::find_closest_index(arr_sorted, 10),
                          std::runtime_error);
    }
    ...
    SECTION("Test case 2: Array with one element - exact match") {
        ...
        size_t val      = 10;
        size_t expected = 0;
        size_t result   = dm_utils::find_closest_index(arr_sorted, val);
